import os
import glob
import paramiko
from dotenv import load_dotenv

# Load environment variables from .env
load_dotenv()

def deploy_stage():
    host = os.getenv("SFTP_HOST")
    user = os.getenv("SFTP_USER")
    password = os.getenv("SFTP_PASS")
    remote_path = os.getenv("SFTP_PATH_STAGE", "/htdocs/stage")
    
    if not all([host, user, password, remote_path]):
        print("Missing SFTP credentials in .env file.")
        return

    # Files to upload
    files_to_upload = glob.glob("*.html") + [
        "style.css", 
        "robots.txt",
        "sitemap.xml",
        "consent.js",
        ".htaccess"
    ]
    
    try:
        print(f"Connecting to {host} via SFTP for Staging Deployment...")
        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh.connect(host, username=user, password=password)
        
        sftp = ssh.open_sftp()
        print(f"Successfully connected. Uploading to staging path {remote_path}...")
        
        # Ensure staging remote directory exists
        try:
            sftp.chdir(remote_path)
        except IOError:
            print(f"Remote staging directory {remote_path} does not exist. Creating it...")
            sftp.mkdir(remote_path)
            sftp.chdir(remote_path)

        # Upload root files
        for filename in files_to_upload:
            if os.path.exists(filename):
                print(f"Uploading {filename} to staging...")
                sftp.put(filename, filename)

        # Upload images directory
        if os.path.exists("images"):
            remote_images_dir = "images"
            try:
                sftp.mkdir(remote_images_dir)
            except IOError:
                pass # Already exists
            
            print("Uploading images directory to staging...")
            for img_file in os.listdir("images"):
                local_img_path = os.path.join("images", img_file)
                if os.path.isfile(local_img_path):
                    sftp.put(local_img_path, os.path.join(remote_images_dir, img_file))

        # Upload fonts directory
        if os.path.exists("fonts"):
            remote_fonts_dir = "fonts"
            try:
                sftp.mkdir(remote_fonts_dir)
            except IOError:
                pass # Already exists
            
            print("Uploading fonts directory to staging...")
            for font_file in os.listdir("fonts"):
                local_font_path = os.path.join("fonts", font_file)
                if os.path.isfile(local_font_path):
                    sftp.put(local_font_path, os.path.join(remote_fonts_dir, font_file))

        sftp.close()
        ssh.close()
        print("\nStaging deployment successful! Staging live at: https://vellua-music.com/stage/")
        
    except Exception as e:
        print(f"\nStaging deployment failed: {str(e)}")

if __name__ == "__main__":
    deploy_stage()
